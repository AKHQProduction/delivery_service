...
class AccessService:
    ...
    async def _is_shop_owner(self, shop_id: ShopId) -> bool:
        actor = await self._identity_provider.get_user()
        employee = await self._employee_reader.by_identity(actor.user_id)

        return (
            employee.role == EmployeeRole.ADMIN and employee.shop_id == shop_id
        )
    ...
    async def ensure_can_edit_shop(self, shop_id: ShopId) -> None:
        if not await self._is_shop_owner(shop_id):
            raise AccessDeniedError()

    async def ensure_can_delete_shop(self, shop_id: ShopId):
        if not await self._is_shop_owner(shop_id):
            raise AccessDeniedError()
