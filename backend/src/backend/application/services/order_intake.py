...
from backend.application.services.order_payment import apply_balance_payment
...
class OrderIntakeItem:
    product_id: ProductId
    quantity: int


@dataclass(frozen=True)
class OrderIntakeRequest:
    client_id: ClientId
    delivery_date: date
    time_slot_id: TimeSlotId
    address_id: AddressId
    phone_id: PhoneId
    products: list[OrderIntakeItem]
    payment_method: str
    comment: str | None = None
    recurring_order_id: RecurringOrderId | None = None
    geocode_missing_address: bool = True
    insert_into_route: bool = True
...
class OrderIntake:
    ...
    async def create(
        self, shop_id: ShopId, request: OrderIntakeRequest
    ) -> Order:
        client = ensure_exists(
            await self._client_gateway.load(request.client_id),
            "Client",
        )
        if client.shop_id != shop_id:
            raise AccessDeniedError

        time_slot = await self._load_time_slot(request.time_slot_id, shop_id)
        products = await self._resolve_products(request.products, shop_id)

        delivery_address = resolve_address(client, request.address_id)
        if (
            request.geocode_missing_address
            and delivery_address.coordinates is None
        ):
            await self._geocode_delivery_address(
                shop_id=shop_id,
        ...
        order = create_order(
            ...
            delivery_phone=resolve_phone(client, request.phone_id),
            delivery_address=delivery_address,
            payment_method=request.payment_method,
            comment=request.comment,
            time_slot_id=request.time_slot_id,
            recurring_order_id=request.recurring_order_id,
        )
        order.items = [
            create_order_item(
                name=product.name,
        ...
        if is_balance_payment_method(name=order.payment_method):
            client_for_update = ensure_exists(
                await self._client_gateway.load_for_update(client.id),
                "Client",
            )
            apply_balance_payment(order, client_for_update)

        if request.insert_into_route:
    ...
    async def _load_time_slot(
        self, time_slot_id: TimeSlotId, shop_id: ShopId
    ) -> ShopDeliveryTimeSlot:
    ...
    async def _resolve_products(
        self, products: list[OrderIntakeItem], shop_id: ShopId
        ...
        for product_id in product_ids:
            product = ensure_exists(products_map.get(product_id), "Product")
    ...
    async def _geocode_delivery_address(
        self,
        *,
        shop_id: ShopId,
        address_id: AddressId,
        ...
        addr_obj = next(
            address for address in client.addresses if address.id == address_id
        )
    ...
    async def _auto_insert_into_route(
        ...
        if route_plan is None:
            ...
            if all(existing.id != order.id for existing in all_orders):
    ...
    async def _create_initial_route_plan(
        ...
        if all(existing.id != order.id for existing in existing_orders):
