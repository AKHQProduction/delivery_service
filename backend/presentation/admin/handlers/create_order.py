...
async def on_select_delivery_date(
    ...
    current_date: date = datetime.datetime.now(tz=datetime.timezone.utc).date()
    ...
    if not (
        ...
        return await call.answer(
            "Замовлення приймаются в рамках поточного місяця"
        )
...
async def on_input_user_address(
    ...
    if not is_address_specific_enough(msg.text) and is_contains_emoji(
        msg.text
    ):
        await waiting_msg.delete()

        return await msg.answer("🙅‍♀️ Будь ласка, введіть корректну адресу")
...
async def get_create_order_dialog_data(
    ...
    **_kwargs,
    ...
    dialog_manager.dialog_data["apartment"] = dialog_manager.find(
        "apartment_input",
    ).get_value()
...
async def on_reject_order(
    call: CallbackQuery, _: Button, manager: DialogManager
):
...
create_order_dialog = Dialog(
    ...
    Window(
        ...
        CustomCalendar(id="delivery_date", on_click=on_select_delivery_date),
        state=states.CreateOrder.DELIVERY_DATE,
    ),
    Window(
        Const("4️⃣ <b>Коли саме Ви хочете отримати замовлення</b>"),
        Button(
            Const(MORNING_TEXT), id="morning", on_click=on_select_delivery_time
        ),
        Button(
