...
async def staff_workflow_btn(_: Message, dialog_manager: DialogManager):
...
async def get_user_from_staff(
    ...
    **_kwargs,
    ...
    result = await action(
        GetUsersInputData(pagination=Pagination(), filters=GetUsersFilters()),
    )
    ...
    Window(
        ...
        ScrollingGroup(
            ...
            Select(
                ...
                text=Format("{item.full_name} | {item.role.value}"),
                items="user",
