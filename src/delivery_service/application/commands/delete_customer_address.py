...
class DeleteCustomerAddressRequest(TelegramRequest[None]):
    address_id: AddressID
    customer_id: CustomerID


class DeleteCustomerAddressHandler(
    RequestHandler[DeleteCustomerAddressRequest, None]
):
    def __init__(
        self,
    ...
    async def handle(self, request: DeleteCustomerAddressRequest) -> None:
        logger.info(
            "Request to delete customer address %s", request.customer_id
        )
        ...
        customer_registry.delete_customer_address(
            customer, request.address_id, current_user_id
        )

        logger.info(
            "Successfully delete customer address %s", request.customer_id
        )
