...
export const EditOrderForm: React.FC<EditOrderFormProps> = ({
  ...
  const { clients, getClients, loadMoreClients, loadingMore: clientsLoadingMore, hasMore: clientsHasMore } = useClient();
  const { products, getProducts, loadMoreProducts, loadingMore: productsLoadingMore, hasMore: productsHasMore } = useProducts();
  ...
  useEffect(() => {
    if (order && clients.length > 0 && products.length > 0) {
      ...
      setSelectedAddressId(order.address_id || client?.addresses?.[0]?.id || null);

      const items: OrderItem[] = order.items?.map((item: any) => ({
        id: item.id,
        product_id: item.product_id,
        name: item.name || products.find((p) => p.product_id === item.product_id)?.name || "",
        price: item.price_per_item || item.price || products.find((p) => p.product_id === item.product_id)?.price || 0,
  ...
  const handleSubmit = async () => {
    if (!selectedClient || !selectedPhoneId || !selectedAddressId || orderItems.length === 0) {
              ...
              {selectedClient ? (
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-sm text-gray-500">{selectedClient.phones?.[0]?.number || "—"}</div>
                  </div>
                  ...
                  clients.map((client) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-sm text-gray-500">{client.phones?.[0]?.number || "—"}</div>
                      </div>
                      {selectedClient?.client_id === client.client_id && (
